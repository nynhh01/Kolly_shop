<?php
    include('../db/connect.php');
    
    if(isset($_POST['add_product'])){
        $name_tree=$_POST['name_tree'];
        $cost_current=$_POST['cost_current'];
        $cost_old=$_POST['cost_old'];
        $description_tree=$_POST['description_tree'];

        $img_main=$_FILES['img_main']['name'];
        $img_main_tmp=$_FILES['img_main']['tmp_name'];
        $img_main= time().'_'.$img_main;
        
        $img_sub1=$_FILES['img_sub1']['name'];
        $img_sub1_tmp=$_FILES['img_sub1']['tmp_name'];
        $img_sub1= time().'_'.$img_sub1;

        $img_sub2=$_FILES['img_sub2']['name'];
        $img_sub2_tmp=$_FILES['img_sub2']['tmp_name'];
        $img_sub2= time().'_'.$img_sub2;

        $img_sub3=$_FILES['img_sub3']['name'];
        $img_sub3_tmp=$_FILES['img_sub3']['tmp_name'];
        $img_sub3= time().'_'.$img_sub3;

        $img_sub4=$_FILES['img_sub4']['name'];
        $img_sub4_tmp=$_FILES['img_sub4']['tmp_name'];
        $img_sub4= time().'_'.$img_sub4;
        
        move_uploaded_file($img_main_tmp,'../img_upload/'.$img_main);
        move_uploaded_file($img_sub1_tmp,'../img_upload/'.$img_sub1);
        move_uploaded_file($img_sub2_tmp,'../img_upload/'.$img_sub2);
        move_uploaded_file($img_sub3_tmp,'../img_upload/'.$img_sub3);
        move_uploaded_file($img_sub4_tmp,'../img_upload/'.$img_sub4);

        $sql="INSERT INTO products(name_tree,cost_current,cost_old,description_tree,img_main,img_sub1,img_sub2,img_sub3,img_sub4) 
        VALUES('$name_tree','$cost_current','$cost_old','$description_tree','$img_main','$img_sub1','$img_sub2','$img_sub3','$img_sub4')";
        if($mysqli->query($sql)===true){
            echo '<script>alert("Thêm thành công.")</script>';
        }else{
            echo '<script>alert("Đã có lỗi xảy ra. Vui lòng kiểm tra lại.")</script>';
        }
    }
    $sql_lk="SELECT * FROM products";
    $query_lk= mysqli_query($mysqli,$sql_lk);
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Kolly | Admin</title>
    <link rel="shortcut icon" href="data:image/png;base64," type="image/x-icon">
    <link rel="stylesheet" href="admin_style.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif:wght@400;700&display=swap" rel="stylesheet">
    <script src="https://kit.fontawesome.com/4f34545577.js" crossorigin="anonymous"></script>
</head>
<body>

    <header class="header">
        <p>Trang quản lý Page Kolly</p>
    </header>
    <section class="admin">
        <div class="control__left">
            <ul>
                <li><a href="?file=products">Sản phẩm</a>
                    <ul class="left__item">
                        <li><a href="">Thêm sản phẩm</a></li>
                        <li><a href="">Danh sách sản phẩm</a></li>
                    </ul>
                </li>
                <li><a href="../main/?file=login">Đăng xuất</a></li>
            </ul>
        </div>
        <div class="control__right">
            <div class="add__products">
                <p class="add__products--title">Thêm Sản phẩm</p>
                <form action="" autocomplete="off" method="post" enctype="multipart/form-data">
                        <table>
                        <tr>
                            <td>Tên sản phẩm</td>
                            <td><input type="text" name="name_tree" id=""></td>
                        </tr>
                        <tr>
                            <td>Giá hiện tại</td>
                            <td><input type="text" name="cost_current" id=""></td>
                        </tr>
                        <tr>
                            <td>Giá cũ</td>
                            <td><input type="text" name="cost_old" id=""></td>
                        </tr>
                        <tr>
                            <td>Mô tả</td>
                            <td><input type="text" name="description_tree" id=""></td>
                        </tr>
                        <tr>
                            <td>Hình ảnh chính</td>
                            <td><input type="file" name="img_main" id=""></td>
                        </tr>
                        <tr>
                            <td>Hình ảnh phụ 1</td>
                            <td><input type="file" name="img_sub1" id=""></td>
                        </tr>
                        <tr>
                            <td>Hình ảnh phụ 2</td>
                            <td><input type="file" name="img_sub2" id=""></td>
                        </tr>
                        <tr>
                            <td>Hình ảnh phụ 3</td>
                            <td><input type="file" name="img_sub3" id=""></td>
                        </tr>
                        <tr>
                            <td>Hình ảnh phụ 4</td>
                            <td><input type="file" name="img_sub4" id=""></td>
                        </tr>
                    </table>
                        <input type="submit" class="add_product__sub" name="add_product" value="Thêm sản phẩm">

                    </form>
            </div>

        </div>
    </section>
    <table>
        <tr>
            <td>Tên sản phẩm</td>
            <td>Giá hiện tại</td>
            <td>Mô tả</td>
            <td>Hình ảnh chính</td>
            <td>Hình ảnh phụ 1</td>
        </tr>

        <?php
            while($row = mysqli_fetch_array($query_lk)){
        ?>
            <tr>
                <td><p class="name"><?php echo $row['name_tree'] ?></p></td>
                <td><p class="cost"><?php echo $row['cost_current'] ?></p></td>
                <td><p class=""><?php echo $row['description_tree'] ?></p></td>
                <td><img src="../img_upload/<?php echo $row['img_main'] ?>" alt="" class="img1" width="100px"></td>
                <td><img src="../img_upload/<?php echo $row['img_sub1'] ?>" alt="" class="img2" width="100px"></td>
                <td><a href="?file=delete&id=<?php echo $row['id_pro'] ?>">Xóa</a></td>
            </tr>
        <?php
            }
         ?>
     </table>
</body>
</html>