<?php
    ...
    if(isset($_POST['login'])){
        $nameuser=$_POST['idlogin'];
        $passuser=$_POST['passlogin'];
        if($nameuser=='admin' && $passuser=='admin'){
            header("Location: ../admin/?file=products");
        }else{
            $sql ="SELECT * FROM login_user WHERE username='$nameuser' AND pass='$passuser'";
        ...
        <div class="login-img1">
            <div class="login-form">
                ...
                <form action="" autocomplete="off" method="post">
                    <div class="box">
                        <input type="text" name="idlogin" id="make" placeholder="Tên đăng nhập" required>
                    </div>
                    <div class="box">
                        <input type="password" name="passlogin" id="make" placeholder="Mật khẩu" required>
                    </div>
                ...
                <div class="last-sub">
                    <p>Bạn chưa có tài khoản ? <a href="?file=signin" class="signin">Đăng ký</a></p>
                    <span class="miss">Quên mật khẩu</span>
                </div>
            </div>
        </div>
    </div>
