<?php
    ...
    $sql_lk=" SELECT * FROM products p inner join carts c on p.id_pro=c.id_product where c.id_order_user= $id_user ";
    $query_lk= mysqli_query($mysqli,$sql_lk);
    $sum="SELECT sum(cost_current) as sum_total from carts c inner join products p on c.id_product=p.id_pro WHERE id_order_user= $id_user";
    $query_sum= mysqli_query($mysqli,$sum);
 ?>
...
<body>
    ...
    <div class="cart__order">
        ...
        ?>
            <div class="cart__pro">
                <img src="../img_upload/<?php echo $row['img_main'] ?>" alt=""  >
                <p class="name--tree_space"><?php echo $row['name_tree'] ?></p>
                <p class="tree__price">Giá: <span ><?php echo $row['cost_current'] ?></span> VNĐ</p>
                <div class="delete_t">
                    <a href="?file=delete_pro&id_user=<?php echo  $row['id_order_user']?>&id=<?php echo $row['id_pro'] ?>" class="delete__tree" >Xóa</a>
                </div>
            </div>
        <?php
            }
        ...
        <div class="total_price">
            <p>Tổng: <span>
                <?php 
                $summ=mysqli_fetch_array($query_sum);
                echo  $summ['sum_total'];
                ?>
            </span>VNĐ</p>
        </div>
        ...
        <div class="buy_pro">
            <a href="?file=order&id_user=<?php echo $id_user?>" class="submit_buy_pro">MUA HÀNG</a>
        </div>
    </div>
